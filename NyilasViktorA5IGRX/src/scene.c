...
double dragon_rotate1=0;
int i =0;
double dragon_moves=0;
double brightness=0;
...
{
    load_model(&(scene->mountain), "obj/oszlop.obj");
    scene->texture_mountain = load_texture("textures/oszlop.jpg"); 	
	
	load_model(&(scene->pine_tree1), "obj/fa.obj");
	scene->texture_pinetree = load_texture("textures/pinetree.jpg");
	
	
		
    load_model(&(scene->pine_tree2), "obj/fa.obj");
    load_model(&(scene->pine_tree3), "obj/fa.obj");
    load_model(&(scene->pine_tree4), "obj/fa.obj");
    load_model(&(scene->pine_tree5), "obj/fa.obj");
    load_model(&(scene->pine_tree6), "obj/fa.obj");
    load_model(&(scene->pine_tree7), "obj/fa.obj");

    load_model(&(scene->dragon), "obj/dragon.obj");
    scene->texture_dragon = load_texture("textures/body.jpg"); 	
  	
	
    load_model(&(scene->ice), "obj/talaj.obj");
...
void draw_scene(const Scene* scene)
{
	...
	glPushMatrix();
		glRotatef(180,0,1,0);
		glTranslatef(0.9,0.0,-0.3);
	draw_model(&(scene->pine_tree4));
	glPopMatrix();	
	
	glBindTexture(GL_TEXTURE_2D, scene->texture_pinetree);	
	glPushMatrix();
		glRotatef(180,0,1,0);
		glTranslatef(1.9,0.0,1.3);
	draw_model(&(scene->pine_tree5));
	glPopMatrix();	
	glBindTexture(GL_TEXTURE_2D, scene->texture_pinetree);	
	glPushMatrix();
		glRotatef(180,0,1,0);
		glTranslatef(2.0,0.0,-0.3);
	draw_model(&(scene->pine_tree6));
	glPopMatrix();	
	glBindTexture(GL_TEXTURE_2D, scene->texture_pinetree);	
	glPushMatrix();
		glRotatef(180,0,1,0);
		glTranslatef(2.0,0.0,-0.3);
	draw_model(&(scene->pine_tree7));
	glPopMatrix();	
	...
	glPushMatrix();
				...
				glRotatef(360+dragon_rotate,0,1,0);
		glTranslatef(-1.3+dragon_x,0+dragon_y,-5.2);
...
void dragon_move(double time){
		...
		/*if(dragon_moves <0.054)
		{*/
			if(dragon_x >=4)
			{
		
		
			dragon_x=dragon_x+time/200;
	dragon_y=dragon_y+time/10;
	dragon_rotate=dragon_rotate-time*10;
	dragon_rotate1 = dragon_rotate;
	
		if(dragon_y>=3)
		{
			dragon_x = dragon_x+time/0.1;
			dragon_y=dragon_y+time/0.5;
			
		}
			}
		else{
		dragon_x = dragon_x+time/0.4;
			dragon_y=dragon_y+time/8;	
		}
